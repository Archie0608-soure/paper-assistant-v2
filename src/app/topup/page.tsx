  ...
  const handleTopup = async () => {
    ...
    try {
      const res = await fetch('/api/topup/initiate', {
        ...
        body: JSON.stringify({ coins: pkg.coins, method: 'wechat' }),
      });
            ...
            <div className="grid grid-cols-2 gap-3">
              {PACKAGES.map((p, i) => (
                <button
                  ...
                  className={"relative p-4 rounded-xl border-2 text-left transition-all " + (selected === i ? 'border-indigo-500 bg-indigo-50 shadow-md' : 'border-slate-200 hover:border-slate-300')}
                >
                  {p.bonus > 0 && (
        ...
        {!qrCode && (
          <div className="bg-white rounded-2xl p-5 shadow-lg border border-slate-200">
            ...
            <button
              onClick={handleTopup}
              disabled={loading}
              className="w-full py-4 bg-green-600 text-white rounded-xl font-semibold text-lg shadow-lg hover:shadow-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
